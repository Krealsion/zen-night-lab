...
//
// One source, two libraries (the Zengine snake-world / probe precedent):
//
...
// vocabulary or the host altered. That is the whole claim this weave exists to
// make testable.
//
...
// is liveness bookkeeping and belongs to the party that watches promises. A
// policy that also tracked stations would be two things, and only one of them
// would be replaceable.
//
// IT ANSWERS, RATHER THAN REPLIES. The expediter asked a ROLE, so it cannot know
// which incarnation received the query and cannot pre-bind the answer's sender.
// Loom's attestation is the only thing that closes that gap, and the expediter
// checks it. (The answer goes out through `answer_across_the_seam` and not
// `Mail::answer`, because this weave is dynamically loaded and `Mail::answer` is
// native-only — read answering.hpp; it is a substrate gap, not a style choice.)
...
public:
    void on(const RouteQuery& q, loom::Mail& mail) {
        ...
        // 4. A REQUIRED preference binds every policy, including this one. That
        //    is not politeness — an empty fallback is the diner saying "this
        //    station or nothing", and a policy that overrode it would be turning
        //    a refusal the diner asked for into a dish they did not.
        if (required) {
            if (!wants_a_station) {
                // "no preference, and no fallback is acceptable" cannot be
                // honoured by anyone: there is nothing to require.
                refuse(mail, q,
...
private:
    ...
    void choose(loom::Mail& mail, const RouteQuery& q, const std::string& station,
        ...
        answer_across_the_seam(mail, RouteChoice{q.order_id, station, resolved,
                                "[" + std::string(kPolicyName) + "] " + std::move(reason)});
    }

    void refuse(loom::Mail& mail, const RouteQuery& q, std::string reason) {
        ++state_.refusals;
        answer_across_the_seam(mail, RouteChoice{q.order_id, "", kRoutedRefused,
                                "[" + std::string(kPolicyName) + "] " + std::move(reason)});
    }
};
