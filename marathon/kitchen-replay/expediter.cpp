...
//
// REPLAY NOTE. This weave is unchanged from Night One except that its three
// immediate answers are now `mail.answer(...)` instead of the deleted
// `answer_across_the_seam()` workaround, and the ticket they return is looked
// at. That the rest needed no adaptation is itself a result: the prepared-
// replacement ceremony happens entirely between the OWNER and a STATION, and the
// party holding the promises never learns it happened.
...
/// kind of thing this substrate exists to make unnecessary.
struct ExpediterState {
    ...
    std::int64_t unanswerable = 0; ///< answers this weave tried to send and could not
    ZEN_EXPOSE();
    ZEN_SHAPE(ExpediterState, 1, ZEN_FIELD(tickets), ZEN_FIELD(open_stations),
              ...
              ZEN_FIELD(held), ZEN_FIELD(unanswerable));
};
...
/// and published for the same reason every other bound here is. Overflow is
/// VISIBLE — a held order that cannot be held is refused with its reason, and a
/// dropped outcome is counted where a diagnostic can find it.
...
public:
    ...
    /// at all, so the order is not accepted: taking a job while knowing the
    /// receipt can never be written is exactly the silent failure this weave
    /// exists to prevent.
    void on(const PlaceOrder& o, loom::Mail& mail) {
        ...
        if (state_.tickets.size() + held_orders_.size() >= kMaxOpenTickets) {
            say(mail, OrderReceipt{o.order_id, kRoutedRefused, "",
    ...
    void on(const RouteChoice& c, loom::Mail& mail) {
        ...
        if (c.resolved == kRoutedRefused || c.station.empty()) {
            answer_receipt(mail, job, OrderReceipt{t->order_id, kRoutedRefused, "", c.reason});
            ++state_.refused;
            forget(job);
            return;
        }
        answer_receipt(mail, job, OrderReceipt{t->order_id, c.resolved, c.station, c.reason});
    ...
    /// A dish is up.
    ///
    /// THE HONEST LIMIT OF THIS CHECK, stated where it lives: a `Plated` is an
    /// ordinary message, and there is no way for this weave to ask Loom whether
    /// its sender holds the station role it claims. The standing consumer
    /// obligation is "match correlation AND bus-stamped sender"; the second half
    /// is IMPOSSIBLE for a role-addressed conversation across replacement,
    /// because the incarnation that plates is legitimately not the one that was
    /// sent the Prep. So this does everything that is available — the job must be
    /// open, past routing, and the claimed station must be the one this job was
    /// actually sent to — and a weave that holds a grant for `Plated` can still
    /// finish someone else's dish. That is measured by the suite's forgery case.
    void on(const Plated& p, loom::Mail& mail) {
        // THE RACE THIS EXPERIMENT ACTUALLY FOUND: a dish plated while the
        // kitchen is changing hands arrives at the HEIR before the heir has been
        // activated, let alone before its letter-claim resolves. Handling it then
        // would find no such job, ignore it as noise, and leave the inherited
        // ticket to time out — a promise broken by the handover itself.
        if (bootstrapping_) {
    ...
    /// so it crosses. The roster crosses with it.
    ///
    /// NOTE FOR THE REPLAY: this handler is reached by the Weave Manager's
    /// graceful swap and by nothing else. A prepared replacement never asks the
    /// incumbent anything, so if this weave were replaced that way none of the
    /// below would run. That asymmetry is the replay's sharpest finding and it is
    /// measured in the suite, not asserted here.
...
private:
    ...
    // race this exists for is a message that arrives BEFORE the activation.
    //
    ...
    // host-native mount would, and nothing here would say so.
    ...
    // without trusting a payload field.

    static std::uint64_t job_correlation(const std::string& job) { return parse_u64(job); }
    ...
    void answer_receipt(loom::Mail& mail, const std::string& job, const OrderReceipt& receipt) {
        for (std::size_t i = 0; i < promises_.size(); ++i) {
            ...
            if (!loom::answer_deferred(promises_[i].answer, mail, receipt).valid()) {
                ++state_.unanswerable;
            }
    ...
    /// THE IMMEDIATE ANSWER, with its ticket looked at. See policy.cpp for why
    /// the return value is not discarded.
    template <class T>
    ...
    void adopt(const loom::Bequest& envelope) {
        ...
        // anything the letter can contain.
